...
banner =''' ____ _______ _______ ______ ______     _     _____ ______    _____ _______ ______ _______ 
...
def getToken(url):
    ...
    data = {"username":"bigsizeme996","loginWay": 1, "loginType": "QQ", "thirdId": "abcdefg"}

    response = requests.post(url=target,headers=headers,json=data,proxies=proxies)
...
def getPath(url,token):
    ...
    response = requests.post(url=target,headers=headers,data=multipart_encoder,proxies=proxies)
